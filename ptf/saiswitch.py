...
class SwitchAttrTest(PlatformSaiHelper):
    ...
    def availableIPv4RouteEntryTest(self):
        ...
        self.available_v4_host_routes = max_route_entry
        routes = dict()
        mask = '/32'
        ...
        try:
            ...
            if self.platform == 'brcm':
                route_entry = sai_thrift_route_entry_t(
                    switch_id=self.switch_id,
                    vr_id = self.default_vrf,
                    destination=sai_ipprefix('fe80::222:bdff:fef8:19ff/0'))
                status = sai_thrift_create_route_entry(
                    self.client, route_entry, next_hop_id=nhop)
                self.assertEqual(status, SAI_STATUS_SUCCESS)
            route_number += 1
    ...
    def availableIPv4NexthopEntryTest(self):
        ...
        try:
            ...
            while nhop_number < max_nhop_entry:
                ...
                self.neigh = sai_thrift_create_neighbor_entry(
                    self.client, self.neigh_entry, dst_mac_address=mac_list[index_num])
                index_num += 1
    ...
    def availableIPv6NeighborEntryTest(self):
        ...
        try:
            ...
            while nbr_number < max_nbr_entry:
                ...
                attr = sai_thrift_get_switch_attribute(
                    self.client, available_ipv6_neighbor_entry=True)
                print("available_ipv6_neighbor_entry: {}, available_nbr_entry: {}, nbr_number: {} ".format(attr["available_ipv6_neighbor_entry"], available_nbr_entry, nbr_number))
...
@group("draft")
class SwitchVxlanTest(PlatformSaiHelper):
    '''
...
@group("draft")
class SwitchDefaultVlanTest(PlatformSaiHelper):
    """
